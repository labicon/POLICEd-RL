...
debug = False
env = MujocoEnv(target=target, precision=precision, max_magnitude=max_action_magnitude)
...
def constraint_cost(state, action):
    ...
    env.reset()
    costs = tensor(np.apply_along_axis(env.crossing, 1, state[:, -3:]), dtype=dtype).to(device)
...
def main_loop():
    ...
    for i_iter in range(args.max_iter_num):
        ...
        #     print('{}\tT_sample {:.4f}\tT_update {:.4f}\tR_avg {:.2f}\tTest_R_avg {:.2f}\tenv_avg_respect {:.2f}'.format( i_iter, log['sample_time'], t1-t0, log['env_avg_reward'], eval_log['env_avg_reward'], log['env_avg_respect']))              
