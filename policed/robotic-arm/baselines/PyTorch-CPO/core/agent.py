...
def collect_samples(pid, queue, env, policy, custom_reward,
    ...
    env_respect_episode_list = []
    env_completion_episode_list = []
    """
    ...
    while num_steps < min_batch_size:
        ...
        env_respect_episode_list.append(float(episode_constraint_respect))
        env_completion_episode_list.append(float(episode_completed))
    ...
    env.counter += num_episodes
    log['env_total_reward'] = env_total_reward
    log['env_avg_respect'] = sum(env_respect_episode_list)/num_episodes
    log['env_avg_completion'] = sum(env_completion_episode_list)/num_episodes
    log['env_avg_reward'] = env_total_reward / num_episodes
    log['max_reward'] = max_reward
    log['min_reward'] = min_reward
    log['env_reward_ep_list'] = env_reward_episode_list
    print(f"Episode: {env.counter} \t Average reward: {log['env_avg_reward']:.2f} \t Average respect: {log['env_avg_respect']:.2f} \t Percent Completion: {log['env_avg_completion']:.2f} %")
...
def merge_log(log_list):
    ...
    try:
        log['env_avg_respect'] = log['env_avg_respect']
    except:
        log['env_avg_respect'] = -1.0
    for x in log_list:
    ...
    reward_episode_list_array = np.array(total_rewards_episodes) - log['env_avg_reward'].numpy()
...
class Agent:
    ...
    def collect_samples(self, min_batch_size):
        ...
        #     worker_args = (i+1, queue, self.policy, self.custom_reward, self.mean_action, False, self.running_state, thread_batch_size)
        #     workers.append(multiprocessing.Process(target=collect_samples, args=worker_args))
        # for worker in workers:
        #     worker.start()

        memory, log = collect_samples(pid=0, queue=None, env=self.env, policy=self.policy, custom_reward=self.custom_reward, mean_action=self.mean_action, render=self.render, running_state=self.running_state, min_batch_size=thread_batch_size)
                                                            # MujocoEnv()
